...
$(document).ready(function () {
    ...
    ajax: {
      ...
      dataSrc: function (json) {
        if (json.sql) {
          Swal.fire({
            title: 'SQL Executado',
            html: `<pre style="text-align:left; white-space:pre-wrap;">${json.sql}</pre>`,
            icon: 'info'
          });
        }
        return json.pedidos || [];
  ...
  $('#tabelaPedidos tbody').on('click', 'tr', function (e) {
    if ($(e.target).closest('.btn-excluir').length > 0) return;
      ...
      success: function (response) {
        if (response.status === 'success') {
          if (response.sql) {
            Swal.fire({
              title: 'SQL Executado',
              html: `<pre style="text-align:left; white-space:pre-wrap;">${response.sql}</pre>`,
              icon: 'info'
            });
          }
