...
<script>
    $(document).ready(function () {
        ...
        function ativarEventos() {
            $('.pizza-checkbox').on('change', function () {
                ...
                if (!ativo) {
                    ...
                    const qtdAtual = linha.find('.pizza-quantidade').val();
                    if (!qtdAtual || qtdAtual == 0) {
        ...
        function extrairJSON(texto) {
            ...
            if (resultado) {
                return JSON.parse(resultado[0]);
            }
            throw new Error('JSON não encontrado no texto');
        }

        // Carregar pedido, clientes e pizzas
        $.post('controller.php', { funcao: 'buscarPedido', id_pedido: pedidoId }, function(resPedido) {
            try {
                const dados = JSON.parse(resPedido);
                if (dados.status !== 'success') {
                    Swal.fire('Erro', dados.mensagem || 'Erro ao carregar pedido.', 'error');
                    return;
                }
                ...
                $.post('controller.php', { funcao: 'listarClientes' }, function(resClientes) {
                    try {
                        const clientes = extrairJSON(resClientes);
                        $('#cliente_id').empty();
                        clientes.forEach(c => {
                        ...
                        $.post('controller.php', { funcao: 'listarPizzas' }, function(resPizzas) {
                            try {
                                const pizzas = extrairJSON(resPizzas);
                                $('#lista-pizzas').empty();
                                pizzas.forEach(p => {
                                    ...
                                    const tamanho = selecionada ? selecionada.tamanho : 'Pequena';
                                ...
                                Swal.fire('Erro', 'Falha ao carregar pizzas.', 'error');
                            }
                        });
                    } catch (e) {
                ...
                Swal.fire('Erro', 'Falha ao carregar pedido.', 'error');
            }
        });
